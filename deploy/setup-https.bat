...
powershell -NoProfile -Command "try { $p='%ROOT%\.env'; $c=[System.IO.File]::ReadAllLines($p); $c=$c -replace '^APP_URL=.*$','APP_URL=https://%SITE%'; $c=$c -replace '^SESSION_SECURE_COOKIE=.*$','SESSION_SECURE_COOKIE=true'; [System.IO.File]::WriteAllLines($p, [string[]]$c); exit 0 } catch { Write-Host ('      ' + $_.Exception.Message); exit 1 }"
...
powershell -NoProfile -Command ^
  "try { $p='%CONF%'; $c=[System.IO.File]::ReadAllLines($p);" ^
  ...
  "[System.IO.File]::WriteAllLines($p, [string[]]$c); exit 0 } catch { Write-Host ('      ' + $_.Exception.Message); exit 1 }"
...
REM An empty HOSTIP would leave a bare "ServerAlias" with no argument, which
REM Apache refuses to start on. Comment the line out instead of emitting a
REM broken one; the name still works, only the by-IP route is lost.
set ALIASREP=ServerAlias %HOSTIP%
if "%HOSTIP%"=="" set ALIASREP=# ServerAlias - this machine's IP was not detected

set FSROOT=%ROOT:\=/%
powershell -NoProfile -Command ^
  "try { $c=[System.IO.File]::ReadAllText('%TEMPLATE%');" ^
  "$c=$c -replace 'C:/xampp/htdocs/lms','%FSROOT%';" ^
  "$c=$c -replace 'Require ip 192\.168\.254\.0/24','Require ip %SUBNET%';" ^
  "$c=$c -replace 'ServerAlias SERVER_IP_HERE','%ALIASREP%';" ^
  "$c='# GENERATED by deploy\setup-https.bat - edit apache-vhost.conf instead.' + [Environment]::NewLine + $c;" ^
  "[System.IO.File]::WriteAllText('%LOCALVHOST%', $c); exit 0 } catch { Write-Host ('      ' + $_.Exception.Message); exit 1 }"
